...
import { Location, Calendar } from '@/src/components/icons';
...
const Cards = () => {
  ...
  useEffect(() => {
    const getEvents = async () => {
      const response = await axios.get('https://prod-strapi.jkyog.org/api/web-app-blogs?populate=poster');
          ...
          >
            <div className="flex items-start gap-2">
              <div className="flex items-center gap-1 px-2 py-1.5 rounded-3xl bg-[#fef4ed]">
                <Location />
                <p className="text-[#EF6F1F] text-center text-xs not-italic font-bold leading-4">5 minread</p>
              </div>
              <div className="flex items-center gap-1 px-2 py-1.5 rounded-3xl bg-[#fef4ed]">
                {/* <Calendar /> */}
